<template>
  <BaseWidgetLayout :content-title="$t('Select Checkpoint')">
    ...
    <template #content>
          ...
          <template #bottom>
            <CardBottom>
              <div class="p-2">
                ...
                <p class="text-xs text-neutral-600 dark-theme:text-neutral-400 truncate mt-1">
                  {{ asset.filename }}
                </p>
              </div>
            </CardBottom>
      ...
      <div v-if="filteredAssets.length === 0 && !assetStore.isLoading" class="text-center py-8">
        <p class="text-neutral-500">No checkpoints found</p>
      </div>

      <div v-if="assetStore.isLoading" class="text-center py-8">
        <p class="text-neutral-500">Loading checkpoints...</p>
      </div>
    </template>
  </BaseWidgetLayout>
</template>
...
onMounted(() => {
  assetStore.loadCheckpointAssets()
})
