...
export function createRetryStateMachine<T = any>(
  ...
  return createConditionStateMachine({
    ...
    context,
    debug: config.debug,
    checkInterval: 50, // Fast checking for responsiveness
...
async function defaultSystemReadinessCheck<T>(
  ...
  const minBackoffTime = Math.min(100 * Math.pow(2, ctx.attemptCount - 1), 1000)
...
export class RetryStateMachine<T = any> {
  ...
  private async transitionTo(targetState: RetryState): Promise<void> {
    // This is a simplified transition - in a more complex implementation,
    // we might need to validate transitions
    const currentState = this.currentState

    // Update the state machine's internal state
    // Note: This is a bit of a hack since we're bypassing the condition checks
    // In a production system, we might want a more structured approach
    this.stateMachine['_currentState'] = targetState

    if (this.context.debug) {
      console.log(
        `[RetryStateMachine] Transition: ${currentState} -> ${targetState}`
      )
    }
  }
}
